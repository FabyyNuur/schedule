const classes= localStorage.getItem('classes') ? JSON.parse(localStorage.getItem('classes')):[
    ...
    {id:6, nom: 'L2 CDSD', effectif: 4, semaine: '', planning: [[],[],[],[],[],[]]},
];
const Modules= localStorage.getItem('Modules') ? JSON.parse(localStorage.getItem('Modules')):[
...
const enseignants= localStorage.getItem('enseignants') ? JSON.parse(localStorage.getItem('enseignants')):[
    {id:1, nom: 'Aly', semaine: '', planning: [[],[],[],[],[],[]], modules:[1,3,4,5]},
    {id:2, nom: 'Baila', semaine: '', planning: [[],[],[],[],[],[]], modules:[1,2,5,6]},
    {id:3, nom: 'Ndoye', semaine: '', planning: [[],[],[],[],[],[]], modules:[1]},
    {id:4, nom: 'Mbaye', semaine: '', planning: [[],[],[],[],[],[]], modules:[3,5,4]},
    {id:5, nom: 'Djiby', semaine: '', planning: [[],[],[],[],[],[]], modules:[1,2,6]},
    {id:6, nom: 'Seckouba', semaine: '', planning: [[],[],[],[],[],[]], modules:[5,6]},
];
const Salles=localStorage.getItem('Salles') ? JSON.parse(localStorage.getItem('Salles')):[
...
//-------------------------------------------
let Classe =localStorage.getItem('Classe') ? JSON.parse(localStorage.getItem('Classe')): {
    id: 12,
    nom: 'L2 CDSD',
    effectif: 43,
    ...
    planning: [
        ...
        [],
        [
            {module: 'JAVASCRIPT', prof: 'Aly', duree: 2, debut: 15,salle: '102'},
        ],
        [
...
let Enseignant =localStorage.getItem('Enseignant') ? JSON.parse(localStorage.getItem('Enseignant')): {
    id:1, 
    nom: 'Aly', 
    semaine: '16/01/2023 - 21/01/2023', 
    planning: [
...
let Module =localStorage.getItem('Module') ? JSON.parse(localStorage.getItem('Module')): {
    id:1, 
    nom: 'ALGO', 
    semaine: '16/01/2023 - 21/01/2023', 
    planning: [
...
let Salle=localStorage.getItem('Salle') ? JSON.parse(localStorage.getItem('Salle')):{
    id:1, 
    nom: '101', 
    semaine: '', 
    planning: [
...
// ---------------------------------------
const colors = ['rgb(66, 183, 237)','rgb(11, 170, 14)','#F68C00','#ec6c6fdf','#7FB8B4','#9F4C9D','#0073BC','#e14fd3','#ee9c9db0','#58e5f851'];
...
btnSaveModal.addEventListener('click',()=>{
    ...
    if (cours.debut>(+getSelectedValue(selectFin))) {
        ...
        if (getSelectedValue(selectSalle)!=0 && findW(Salles,getSelectedValue(selectSalle)).nbrPlaces<Classe.effectif) {
            errorMod.innerHTML='Cette Salle ne peut contenir l effectif de cette classe..'
            cours={}
        }else{
            errorMod.innerHTML=''
            if (getSelectedValue(selectSalle)!=0 && verifSalleEff((+jour)-1,(+getSelectedValue(selectDebut)),findW(Salles,getSelectedValue(selectSalle)).nom,(+getSelectedValue(selectFin)))==1) {
                errorMod.innerHTML='Cette Salle est occupe a cet heure..'
            }else{
                errorMod.innerHTML=''
                if (verifProfCours((+jour)-1,(+getSelectedValue(selectDebut)),findW(enseignants,getSelectedValue(selectEnseignant)).nom,(+getSelectedValue(selectFin)))==1) {
                    errorMod.innerHTML='Ce prof a deja un cours a cet heure..'
                }else{
                    if (verifcoursCl((+jour)-1,(+getSelectedValue(selectDebut)),(+getSelectedValue(selectFin)),Classe.nom)==1) {
...
inputt.addEventListener('keyup',()=>{
    ...
    const result1=classes.filter(item=>item.nom.toLowerCase().includes(val.toLowerCase()));
    const result2=Modules.filter(item=>item.nom.toLowerCase().includes(val.toLowerCase()));
    const result3=Salles.filter(item=>item.nom.toLowerCase().includes(val.toLowerCase()));
    const result4=enseignants.filter(item=>item.nom.toLowerCase().includes(val.toLowerCase()));
    let suggestion = ''
...
function afficherPlanningProfs(){
    ...
    Enseignant.planning.forEach((p,i) => {
        ...
        p.forEach(c => {
            let posColor = Math.floor(Math.random() * colors.length);
            jour.appendChild(createDivProfs(c.debut,c.duree,colors[posColor],c.module,c.salle,c.classe));
        });
    });
...
function afficherPlanningMod(){
    ...
    Module.planning.forEach((p,i) => {
        ...
        p.forEach(c => {
            let posColor = Math.floor(Math.random() * colors.length);
            jour.appendChild(createDivMod(c.debut,c.duree,colors[posColor],c.prof,c.salle,c.classe));
        })
    })
}
function afficherPlanningSal(){
    ...
    Salle.planning.forEach((p,i) => {
        ...
        p.forEach(c => {
            let posColor = Math.floor(Math.random() * colors.length);
            jour.appendChild(createDivSalle(c.debut,c.duree,colors[posColor],c.prof,c.module,c.classe));
        })
    })
}
